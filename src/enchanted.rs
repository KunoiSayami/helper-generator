...
use syn::{spanned::Spanned, Attribute, DataEnum, DeriveInput, Fields, Variant};
...
impl FieldsType {
        ...
        match self {
            FieldsType::Named(v) => {
                ...
                if let Some(ret) = return_type {
                    let ident = Ident::new("__private_callback", ret.span());
    ...
    fn into_enchant_arg(&self, span: Span) -> TokenStream {
        let private_receiver = Ident::new("__private_s", span);
        match self {
            FieldsType::Named(v) => {
                let mut idents = v
                    .iter()
...
fn extract_enum(st: &syn::DeriveInput) -> syn::Result<&DataEnum> {
    match st.data {
    ...
    if !no_async {
        let q = quote! {
            pub async fn #function_name (&self, #arg_def) -> std::option::Option< #return_type > {
                let (__private_s, __private_r) = tokio::sync::oneshot::channel();
                self.sender
                    .send(#basic::#member #arg)
                    .await
                    .ok();
...
fn early_check(st: &DeriveInput) -> syn::Result<()> {
    match st.data {
        syn::Data::Enum(_) => {
...
pub fn handle_new(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    if let Err(e) = early_check(&early_st) {
